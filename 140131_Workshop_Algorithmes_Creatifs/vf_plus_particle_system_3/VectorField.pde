  ...
  void draw() {
    ...
    for (int i = 0; i < v.length; i++) {
      for (int j = 0; j < v[0].length; j++) {
        ...
        beginShape(LINES);
        stroke(150);
        vertex((i+.5)*dx, (j+.5)*dy);
        stroke(150, 0);
        vertex((i+.5)*dx+v[i][j].x*l, (j+.5)*dy+v[i][j].y*l);
        endShape();
        ...
        if (rot[i][j] > 5) { // in order to save frames
          ...
          for (int n = 0; n < rot[i][j]*.75; n++) {
            ...
            beginShape();
            fill(250, 250);
            vertex(rot[i][j]*1.5, 0);
            fill(250, 0);
            vertex(rot[i][j]*1.5+rot[i][j]*0.5, 0);
            fill(250, 0);
            vertex(rot[i][j]*1.5+rot[i][j]*0.5, 0+rot[i][j]*0.25);
            fill(250, 250);
            vertex(rot[i][j]*1.5, 0+rot[i][j]*0.25);
            endShape();*/
