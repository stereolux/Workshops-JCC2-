...
RShape shpDiff;
//----------------------------
RShape addCircleRec(RShape rs, int n) {
  if (n==0){
    return(rs);
  }
  else{
    return(addCircleRec( addCircle(rs),n-1));   
  }  
}//end method
//-----------------------------
int randomX = 30;
...
RShape addCircle(RShape rs) {
 // RPoint center = shp.getCentroid(); 
  ...
  RShape shpDiff = RG.union(rs, shpCircle);
  return(shpDiff);
}//end method
...
void glyphTreat(char c) {
  ...
  try {  
    ...
    shpDiff =  addCircleRec(shp,NVirus);  //<<<<<<<<<
